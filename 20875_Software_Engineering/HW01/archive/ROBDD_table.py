...
class Node:
    ...
    def __repr__(self):
        if self.var_name in ('True', 'False'):
            return f'\nNode <{id(self)}>\n\tvar_name={self.var_name}'
        return f'\nNode <{id(self)}>\n\tvar_name={self.var_name}\n\tlow={self.low.var_name}\n\thigh={self.high.var_name} '


def make_node(p, u0, u1):
    
    if (p, u0, u1) in reverseMap:
        return reverseMap[(p, u0, u1)]
    ...
    u = Node(p, u0, u1)
    store.append(u)
    reverseMap[(p, u0, u1)] = u
    return u


def shannon_decomp(expr, var):
    """
    Shannon decomposition of a boolean expression.
...
def build_ROBBD_recursive(expr, vars):
    
    leaf_0 = Node('False', None, None)
    leaf_1 = Node('True', None, None)
    
    global reverseMap, store
    reverseMap = {
        ('False', None, None): leaf_0,
        ('True', None, None): leaf_1
    }
    store = [
        leaf_1,
        leaf_0
    ...
    def build_ROBBD(expr, vars):
        """
        Builds ROBDD from boolean expression.
        ...
        if len(vars) == 1:
            expr0, expr1 = shannon_decomp(expr, vars[0])
            u0 = make_node('True', None, None) if eval(' '.join(expr0)) == True else make_node('False', None, None)
            u1 = make_node('True', None, None) if eval(' '.join(expr1)) == True else make_node('False', None, None)
            return make_node(vars[0], u1, u0)
        
        expr0, expr1 = shannon_decomp(expr, vars[0])
        u0 = build_ROBBD(expr0, vars[1:])
        u1 = build_ROBBD(expr1, vars[1:])
        return make_node(vars[0], u0, u1)
    
    return build_ROBBD(expr, vars)
        ...
        for i in instructions:
            ...
            # ASSIGNMENT -> store in self.ids as {'z': ['x', 'and', 'y']}
                ...
                if (i[0] not in self.vars) and (i[0] not in self.ids):
                    ...
                    self.ids[i[0]] = build_ROBBD_recursive(i[2:], self.vars)
                else:
